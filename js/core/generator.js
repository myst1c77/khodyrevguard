...
function _computeQuickScore(password) {
    ...
    const posBonus = ['length', 'uppercase', 'lowercase', 'numbers', 'special']
        .filter(k => analysis.criteria[k]).length * 2;
...
function _updateGenStats(password) {
    ...
    if (genZxcvbnForScore) {
        const genZxcvbnSec = genZxcvbnForScore.crack_times_seconds.offline_fast_hashing_1e10_per_second;
        const zxcvbnBase = Math.min(100, Math.max(0, Math.round((Math.log10(Math.max(0.001, genZxcvbnSec)) + 3) / 20 * 100)));
