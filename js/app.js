        ...
        function switchPage(page, event) {
            ...
            const titles = { analyzer: 'Анализатор', comparator: 'Сравнитель', mutator: 'Мутатор', generator: 'Генератор', hasher: 'Хеш-генератор' };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('copy', function(e) {
                ...
                if (!['passwordInput', 'generatedPassword', 'compInputA', 'compInputB', 'mutatorInput'].includes(target.id)) {
                    e.preventDefault();
                    return false;
                }
            });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Mutator event listeners
            const mutatorMutateBtn = document.getElementById('mutatorMutateBtn');
            if (mutatorMutateBtn) mutatorMutateBtn.addEventListener('click', runMutations);

            const mutatorInput = document.getElementById('mutatorInput');
            if (mutatorInput) {
                mutatorInput.addEventListener('keydown', e => {
                    if (e.key === 'Enter') runMutations();
                });
            }

            const mutatorToggleBtn = document.getElementById('mutatorToggleBtn');
            if (mutatorToggleBtn) mutatorToggleBtn.addEventListener('click', toggleMutatorVisibility);

            const mutatorResultsEl = document.getElementById('mutatorResults');
            if (mutatorResultsEl) mutatorResultsEl.addEventListener('click', handleMutatorResultsClick);
